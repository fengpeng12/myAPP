...
# 6.5) 固定 debug 签名：AGP 默认用 ~/.android/debug.keystore，不存在时会自动随机生成 →
# 构建机每次都是全新机器，等于每次换签名，同一工程装第二次必被「签名不一致」拒装。
mkdir -p ~/.android
if [ -f debug.keystore.b64 ]; then
  base64 -d debug.keystore.b64 > ~/.android/debug.keystore
  echo "签名：使用固定 debug keystore（每次构建签名一致）"
fi
